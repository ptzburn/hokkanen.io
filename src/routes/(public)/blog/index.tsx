...
import { FeaturePost } from "./_components/feature-post.tsx";
  ...
  return (
    <>
      ...
      <main class="mx-auto w-full py-16">
        <header class="mb-10">
          <div class="flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between sm:gap-6">
            <div>
              <Suspense fallback={<div class="mb-3 h-4" />}>
                <Show when={posts()}>
                  {(list) => (
                    <Show when={list().length > 0}>
                      <p class="mb-3 text-muted-foreground text-xs uppercase tabular-nums tracking-wider">
                        {list().length} {list().length === 1 ? "post" : "posts"}
                        {" "}
                        · {list().reduce((s, p) => s + p.readingMinutes, 0)}
                        {" "}
                        min reading
                      </p>
                    </Show>
              ...
              <h1 class="font-bold text-3xl tracking-tight sm:text-6xl">
                Blog
              </h1>
